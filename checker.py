...
def get_gas_price_gwei():
    try:
        response = requests.post(ARBITRUM_RPC, json=payload, headers=headers, timeout=10)
...
def main():
    ...
    logging.info("Arbitrum Gas Checker запущен...")
    send_telegram_message("🚀 <b>Arbitrum Gas Checker запущен</b>\nОжидаю низкий газ...")
    ...
    while True:
        ...
        message = f"<b>Arbitrum Gas Price</b>\n{status} <code>{gas}</code> gwei"
        ...
        if gas <= GAS_THRESHOLD_GWEI and not notified_low:
            should_notify = True
            notified_low = True
            message = f"🚨 <b>ГАЗ НИЗКИЙ НА ARBITRUM!</b> 🚨\n<code>{gas}</code> gwei\nПора снайпить!"
